...
void TopNRowNumber::processRowBelowLimit<
  ...
  if (topRows.empty()) {
    ...
    if (result > 0) {
      ...
      partition.currentLimit += partition.numTopRankRows();
    }
  }
}
...
RowVectorPtr TopNRowNumber::getOutputFromSpill() {
  ...
  // Rank  of the next row in the current partition.
  ...
  for (;;) {
    ...
    if (index > 0) {
      ...
      if (isNewPartition(output, index, next)) {
        ...
        // peer or not. If it is a new peer (so different order by value) then
        // the ranks are updated.
        if (isNewPeer(output, index, next)) {
          ...
          // This row was a peer with the same order by values as the previous
          // row so the rank remains the same.
          numPeers += 1;
        }

        // Rank is always incremented for Row number function.
        if (rankFunction_ ==
            core::TopNRowNumberNode::RankFunction::kRowNumber) {
          rank += 1;
        }
